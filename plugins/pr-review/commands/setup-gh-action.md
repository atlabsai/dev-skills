---
description: Set up pr-review on GitHub Actions for this repo — checks the repo can run it, adds .github/workflows/pr-review.yml, and walks you through the Claude secret.
...
- Glob `.github/workflows/*.yml` and `*.yaml` and read any that mention `atlabsai/dev-skills`. If one already calls `atlabsai/dev-skills/.github/workflows/pr-review.yml`, say so, show the file path, and stop.
- `gh api repos/<repo>/actions/permissions`:
  ...
  - `"local_only"` → workflows from other repositories are blocked, including this one. The user (or an org admin) must allow `atlabsai/dev-skills/.github/workflows/pr-review.yml@*` under Settings → Actions → General. Stop.
  - `"selected"` → it may be blocked. Tell the user to check that `atlabsai/dev-skills/*` is in the allowed list, then continue.

## 3. Check for a Claude secret

The workflow needs one of two repository secrets: `CLAUDE_CODE_OAUTH_TOKEN` (Claude subscription) or `ANTHROPIC_API_KEY` (API key). Check both places a secret can come from — names only:
...
If either name is present, say which and where, and continue.

If neither is present, tell the user they need to add one, and give both ways:
...
Read `${CLAUDE_PLUGIN_ROOT}/templates/pr-review.yml` and write it, unchanged, to `.github/workflows/pr-review.yml` in the repository. If that path already exists (and wasn't caught in step 2), ask before overwriting it.

Tell the user, in two or three lines, what the workflow does: a review is posted when a PR is opened or marked ready for review; any member can comment `/pr-review` on a PR to re-run it; the optional settings (per-stage models, custom reviewers, self-hosted runners) are commented in the file.
